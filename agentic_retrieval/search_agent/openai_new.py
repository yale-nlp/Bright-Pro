...
def run_conversation_with_tools(client, initial_request: dict, tool_handler: SearchToolHandler, max_iterations: int = 100):
    ...
    for iteration in range(max_iterations):
        ...
        # (not cumulative). Termination via max_iterations / no function_call.
        request = {
...
def main():
    ...
    parser.add_argument('--tasks', type=str, nargs='+', default=None, choices=_SE_TASKS,
                        help='Run multiple SE tasks in one Python process — model loaded once.')
    parser.add_argument('--use-agentic-sample', action='store_true',
                        help='Restrict each task to the fixed 25 qids in agentic_sample_ids.json.')
    parser.add_argument('--max-queries-per-task', type=int, default=None,
                        help='Cap queries per task (smoke testing).')
    parser.add_argument('--task-single', dest='task', type=str, default=None,
        choices=_SE_TASKS, help='Legacy single-task arg (prefer --tasks).')
    ...
    parser.add_argument("--output-dir", default="runs/gpt-5-mini-08-07", help="Directory to store logs (default: %(default)s)")
    ...
    if os.getenv("OPENAI_API_KEY"):
        ...
        print(f"[client] OpenAI public, model={args.model}")
        ...
        print(f"[client] Azure OpenAI, model={args.model}")
    else:
    ...
    print(f"[tasks] {len(tasks_to_run)}: {tasks_to_run}")

    sample_ids_map = {}
    if args.use_agentic_sample:
        _bpro = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
        sample_path = os.path.join(_bpro, 'agentic_sample_ids.json')
        if not os.path.isfile(sample_path):
            raise RuntimeError(f"agentic_sample_ids.json not found at {sample_path}")
        with open(sample_path) as f:
            sample_ids_map = json.load(f)['tasks']
        print(f"[sample] agentic 25-qid/task on")

    args.task = tasks_to_run[0]
    searcher = searcher_class(args)   # MODEL LOADED ONCE
    tool_handler = SearchToolHandler(
        searcher=searcher,
    ...
    print(f"Search agent ({searcher.search_type}) ready; "
          f"k={args.k} snippet_max_tokens={args.snippet_max_tokens}")
